"""Constants for the Techlan Полив integration."""
...
DEFAULT_AUTH_CHANNEL = AUTH_CHANNEL_APP
...
def get_controller_model(controller_type: int | None) -> str:
    ...
    if controller_type is None:
        return "Techlan Полив"
    return CONTROLLER_MODELS.get(controller_type, f"Rain Bird Controller (type {controller_type})")

# ============================================================ камера-детектор осадков
# «Всё про полив и осадки»: облако RainBird IQ4 (app-канал) + зрение по камерам.

# Местное смещение площадки (Белвью, США) для световых режимов day/dusk/night
SITE_UTC_OFFSET = -7
...
# Ключи настроек (entry.options)
CONF_CAMERAS = "cameras"        # [{id,name,url,user,password}]
CONF_ZONES = "zones"            # [{id,name,x,y,w,h,camera}]
CONF_CAM_THRESHOLD = "rain_threshold"
CONF_TRUTH_ENTITY = "truth_entity"      # сенсор дождя-«истина» (binary_sensor)
CONF_INTERLOCKS = "interlocks"          # запреты (как у ворот)
CONF_REACTIONS = "reactions"            # реакции на дождь
CONF_SCAN_CAMERA = "scan_interval_camera"
...
# Панель
PANEL_URL = "rain-techlan"
PANEL_TITLE = "Полив и осадки"
PANEL_ICON = "mdi:weather-rainy"
URL_STATIC = "/rain_techlan"
URL_FRAME = "/api/rain_techlan/frame"
URL_STATE = "/api/rain_techlan/state"
URL_SETTINGS = "/api/rain_techlan/settings"
URL_TEST = "/api/rain_techlan/scan"
