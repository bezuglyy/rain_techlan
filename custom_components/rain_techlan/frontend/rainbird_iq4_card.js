class RainBirdIQ4Card extends HTMLElement {
  ...
  _renderEmpty() {
    return `
      <div class="empty">
        No Techlan Полив zones were found. Add the integration, then refresh this dashboard.
      </div>
    `;
  }
  ...
  _startStation(station, duration) {
    ...
    if (station.mode === "sensor") {
      this._callService("rain_techlan", "start_zone", {
    ...
    this._callService("rain_techlan", "start_station", {
  ...
  _stopStation(station) {
    ...
    this._render(true);
    if (station.mode === "sensor") {
      this._callService("rain_techlan", "stop_zone", {
        station_entity: station.entityId,
      }, {
        error: (error) => this._setStationError(station, error),
      });
    ...
    this._callService("rain_techlan", "stop_station", {
      station_id: Number(station.stationId),
    }, {
      error: (error) => this._setStationError(station, error),
    });
  ...
  _stopAllStations(stations) {
    ...
    byController.forEach((group, controllerId) => {
      ...
      this._render(true);
      this._callService("rain_techlan", "stop_all_zones", {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="editor">
        <div>
          <label>Title</label>
          <input data-key="title" value="${this._escape(this._config.title || "Techlan Полив")}">
        </div>
        <div>
        ...
        <label class="check">
          ...
          Auto-discover Techlan Полив zones
        </label>
        <label class="check">
...
window.customCards.push({
  ...
  name: "Techlan Полив Card",
  description: "Control Techlan Полив zones with per-zone durations and live schedule status.",
});
