...
def _zone_title(station: dict) -> str:
    """«Зона 1» из «Station 001» (или имя станции)."""
    import re as _re
...
async def async_setup_entry(
    ...
    """Set up Techlan Полив sensors from a config entry."""
    ...
    entities.extend([RainTechlanCameraScore(hass, entry), RainTechlanCameraVerdict(hass, entry)])
    async_add_entities(entities)


class RainBirdBaseSensor(CoordinatorEntity, SensorEntity):
    """Base class for Techlan Полив sensors."""
...
class RainBirdAlarmSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: RainBirdCoordinator, config_coordinator: RainBirdConfigCoordinator) -> None:
        ...
        satellite = config_coordinator.data.get("satellite", {}) if config_coordinator.data else {}
        satellite_name = satellite.get("name", "Techlan Полив")
        self._attr_unique_id = f"{self._satellite_id}_alarms"
        self._attr_name = "Тревоги"
...
class RainBirdWarningSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: RainBirdCoordinator, config_coordinator: RainBirdConfigCoordinator) -> None:
        ...
        self._attr_name = "Предупреждения"
...
class RainBirdRainDelaySensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: RainBirdConfigCoordinator) -> None:
        ...
        self._attr_name = "Задержка дождя, дней"
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(
            identifiers={(DOMAIN, str(self._satellite_id))},
            name=satellite.get("name", "Techlan Полив"),
            manufacturer="Rain Bird",
            model=satellite.get("model", "Techlan Полив"),
            sw_version=satellite.get("version"),
        )
...
class RainBirdStationSensor(SensorEntity):
    ...
    def __init__(
        ...
        self._attr_name = _zone_title(station)
...
class RainBirdProgramSensor(SensorEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"Программа {program['shortName']}: статус"
...
class RainBirdControllerModeSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: RainBirdConfigCoordinator) -> None:
        ...
        self._attr_name = "Режим контроллера"
...
class _RainCameraBase(SensorEntity):
    ...
    @property
    def _det(self):
        return _rain_detector(self.hass, self.entry)
    ...
    def _summary(self) -> dict:
        det = self._det
        return (det.last.get("summary") if det else {}) or {}


class RainTechlanCameraScore(_RainCameraBase):
    """Оценка «дождливости» по кадрам камер (0…1)."""

    _attr_name = "Оценка осадков (камера)"
    _attr_icon = "mdi:water-percent"
    _attr_state_class = "measurement"

    def __init__(self, hass, entry) -> None:
        super().__init__(hass, entry)
        self._attr_unique_id = f"{entry.entry_id}_cam_score"
    ...
    def native_value(self):
        ...
        if not det:
            return None
        return float((det.last.get("summary") or {}).get("score") or 0.0)
    ...
    def extra_state_attributes(self):
        ...
        if not det:
            return {}
        st = det.state()
        return {"verdict": st["summary"].get("verdict"), "zone": st["summary"].get("zone"),
                "bucket": st.get("bucket"), "ts": st.get("ts"),
                "results": st.get("results"), "errors": st.get("errors"),
                "threshold": st.get("threshold")}


class RainTechlanCameraVerdict(_RainCameraBase):
    """Вердикт камеры: сухо / возможно / дождь."""

    _attr_name = "Вердикт камеры"
    _attr_icon = "mdi:weather-rainy"

    def __init__(self, hass, entry) -> None:
        super().__init__(hass, entry)
        self._attr_unique_id = f"{entry.entry_id}_cam_verdict"
    ...
    def native_value(self) -> str:
        ...
        if not det:
            return "нет данных"
        return str((det.last.get("summary") or {}).get("verdict") or "нет данных")
