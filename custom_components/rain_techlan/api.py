...
class RainBirdAPI:
    """
    Client for the Techlan Полив REST API.
    ...
    def set_power(self, satellite_id: int, on: bool) -> None:
        """Питание контроллера: логическое положение переключателя (2=On/Auto, 1=Off)."""
        self._patch(
            "Satellite/V2/UpdateBatches",
            json={"ids": [satellite_id],
                  "patch": [{"op": "replace", "path": "/logicalDialPos", "value": 2 if on else 1}]},
        )
